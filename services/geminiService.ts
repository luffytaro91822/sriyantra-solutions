...
// FIX: Initialize the Google Gemini AI client with the API key from environment variables as per guidelines.
...
export const generateInvoiceJson = async (
  ...
  // FIX: Removed the hardcoded API key check which was causing a TypeScript error.
  // The API key is assumed to be provided via environment variables.
